...
@section('content')
<div class="card mb-4">
    <h1 class="h2 mb-0 font-weight-bold card-body text-center">Edit Article</h1>
</div>

<form id="article" data-id="{{ $article->id }}" method="POST" action="{{ route('dashboard.article.update', $article->id) }}">
    @csrf
    {{ method_field('PATCH') }}
            ...
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        <input class="form-control form-control-lg {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" placeholder="Enter title here" value="{{ old('title', $article->title) }}">
                        @if ($errors->has('title'))
                    ...
                    <div style="border: 1px solid #c5c5c5">
                        <textarea class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" name="content" id="content" placeholder="Enter content here" rows="15" required>{{ old('content', $article->content) }}</textarea>
